...
- 新增 `docs/ideas/v18.md`：基于项目源码与完整 research-idea 流程，提出“局部闭包下的人机编程贡献反例历史与最低成本独立见证”方向；明确 certain answers/三值监测不是创新，给出 trust-root、强静态模板、narrow sidecar、4–6 周先导与止损边界。
- 新增 `docs/ideas/BAC_科研战略_v9_从证据边界到干预价值.md`：结合既有方案、BAC 1.3.2 源码与文献，优先研究不完整证据下来源主张的支持边界，保留真实人类干预价值为条件性后续路线；明确独立真值、强基线、停止规则及查新尚未完成的边界。
...
- 为 BAC 科研战略 v9 补充通俗导读：明确近期主攻 AI 编程过程证据审计，从测试执行与代码版本绑定切入，补充三个方向的区别、研究交付物、成对案例、指标释义与先导起步建议；新增内容为建议及示意，不代表已开展实验或重新完成查新。
...
- 重构中英文 README 的叙事结构：先用协作场景解释 BAC 的用途与理念，再展开快速开始、格式、安全边界和开发细节，降低非专业读者的理解门槛。
- 重构 BAC 研究计划：将贡献边界恢复设为唯一主要问题，保留并降级机制、篡改检测、领域复制、反馈复盘、成本与隐私等次级问题，补充实验层级、执行顺序及预先登记的调整/停止规则。
- 为 BAC 研究计划补充真实生产项目观察层：加入授权目录只读盘点、独立真值重建、真实工作片段盲法回放、缺失机制分析和模拟主实验与生产证据的分离推断。
- 调整研究计划实验方案的章节顺序，将完整 S6 生产环境观察与嵌套回放置于 P1 及 S1–S5 支持性实验之后，并明确在阶段 F 最后执行；阶段 B 的生产只读盘点保持为实施前置。
- 扩展生产观察范围至坚果云 MyNotes，并在研究计划中登记 24 个 BAC 账本的实施路径索引；路径仅用于只读定位和复核，实施前需重新扫描。
